    ...
    <div style={{
      ...
      <div style={{
        ...
        <h2 style={{ margin: '10px 0 6px 0', fontSize: '24px', fontWeight: '900' }}>
          🗓️ Day 6 주간 종합 오답 복습 퀴즈 데이
        </h2>
        <p style={{ margin: 0, fontSize: '14px', opacity: 0.9 }}>
          월~금 5일간 공부한 누적 단어 중 오답 단어를 콕 찍어 탈출하는 주간 검증 데이!
        </p>
      </div>
        ...
        <div>
          ...
          <div style={{ background: '#EBF5FB', padding: '18px', borderRadius: '18px', border: '1px solid #AED6F1', marginBottom: '24px' }}>
            ...
            <ul style={{ margin: 0, paddingLeft: '20px', fontSize: '14px', color: '#34495E', lineHeight: '1.6' }}>
              <li>최근 5일간 공부한 누적 단어 중 **자주 틀렸던 오답 단어를 1순위로 자동 추출**합니다.</li>
              <li>복습 퀴즈를 통과하면 **🏵️ [Day 6 주간 오답 탈출 왕도장]**을 수여받게 됩니다!</li>
            </ul>
          </div>
          ...
          >
            🚀 Day 6 주간 종합 오답 탈출 퀴즈 시작하기 (10문항) ➔
          </button>
        </div>
      )}
        ...
        <div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
            <span style={{ fontSize: '14px', fontWeight: 'bold', color: '#6C5CE7' }}>
              문제 {currentQIndex + 1} / {quizQuestions.length}
            </span>
          ...
          {!isAnswerChecked ? (
            ...
            >
              {currentQIndex + 1 < quizQuestions.length ? '다음 복습 문제로 이동 ➔' : '🏵️ Day 6 주간 오답 복습 완수 도장 받기 ➔'}
            </button>
          )}
        </div>
      )}
      ...
      {quizMode === 'quiz_completed' && (
        <div style={{ textAlign: 'center', padding: '20px 10px' }}>
          ...
          <h2 style={{ fontSize: '26px', color: '#2C3E50', fontWeight: '900', margin: '0 0 10px 0' }}>
            축하합니다! Day 6 주간 오답 복습 완수!
          </h2>
          <p style={{ fontSize: '16px', color: '#7F8C8D', margin: '0 0 20px 0' }}>
            최종 점수: <strong style={{ color: '#6C5CE7', fontSize: '20px' }}>{Math.round((score / quizQuestions.length) * 100)}점</strong> ({score} / {quizQuestions.length} 정답)
          </p>
          ...
          <div style={{ background: '#FEF9E7', padding: '20px', borderRadius: '20px', border: '2px solid #FDEBD0', marginBottom: '24px' }}>
            ...
            <p style={{ margin: 0, fontSize: '13px', color: '#7F8C8D' }}>
              클라우드 DB에 기록되었으며, 다음 회차 주간 학습이 연속으로 이어집니다.
            </p>
          </div>
