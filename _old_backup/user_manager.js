...
// 1. 초기 데이터 구조 및 안전한 불러오기
const ADMIN_PIN = '1234'; // 관리자 기본 비밀번호 (부모님/선생님용)

// 로컬 저장소에 저장된 관리자 학생 데이터 불러오기
let storedData = localStorage.getItem('english_student_list');
let studentList = [];
...
// 처음 시작해서 저장된 학생 데이터가 전혀 없을 때만 1회 기본 샘플 등록
...
// 현재 로그인한 학생의 일일 학습 단어 수 가져오기 (기본 20개)
function getCurrentUserWordCount() {
    if (currentUser && currentUser.dailyWordCount) {
        return parseInt(currentUser.dailyWordCount);
    }
    return 20;
}
        ...
        userHeaderBar.innerHTML = `
            <div style="display: flex; justify-content: space-between; align-items: center; background: #FFF9E6; border: 2px solid #FFE66D; border-radius: 16px; padding: 10px 16px; margin-bottom: 16px;">
                <div>
                    ...
                    <span style="font-size: 12px; background: #FFD166; padding: 2px 8px; border-radius: 10px; margin-left: 6px; font-weight: bold; color: #2C3E50;">목표: 하루 ${count}단어</span>
                </div>
                <div>
        ...
        <div style="background: white; border-radius: 24px; padding: 24px; width: 90%; max-width: 400px; box-shadow: 0 10px 30px rgba(0,0,0,0.2); text-align: center;">
            ...
            <div style="margin-bottom: 12px; text-align: left;">
                ...
                <select id="login-student-select" style="width: 100%; padding: 12px; margin-top: 4px; border: 2px solid #E9ECEF; border-radius: 12px; font-size: 15px; font-weight: bold;">
                    ${studentList.map(s => `<option value="${s.id}">${s.name} (${s.grade}) - 하루 ${s.dailyWordCount || 20}단어</option>`).join('')}
                </select>
            </div>
...
function processStudentLogin() {
    ...
    if (targetStudent && targetStudent.pin === inputPin) {
        ...
        alert(`🎉 ${targetStudent.name} 학생, 반갑습니다! 오늘의 목표 ${targetStudent.dailyWordCount || 20}개 단어 공부를 시작해 봅시다!`);
...
// 6. 관리자 모드 모달 (학생 등록 & 하루 학습 단어 수 설정)
...
function renderAdminUI(modal) {
    modal.innerHTML = `
        <div style="background: white; border-radius: 24px; padding: 24px; width: 90%; max-width: 480px; max-height: 85vh; overflow-y: auto; box-shadow: 0 10px 30px rgba(0,0,0,0.2);">
            <h2 style="color: #2C3E50; margin-bottom: 8px; text-align: center;">⚙️ 관리자 모드 (학생 & 학습량 관리)</h2>
            <p style="font-size: 13px; color: #7F8C8D; margin-bottom: 16px; text-align: center;">학생을 등록하고 하루 학습할 단어 수를 설정해 주세요.</p>
            
            <!-- 학생 등록 폼 -->
            <div style="background: #F8F9FA; padding: 16px; border-radius: 16px; margin-bottom: 20px;">
                <h4 style="margin-bottom: 10px; color: #4ECDC4;">➕ 새 학생 추가 등록 및 학습량 설정</h4>
                ...
                <div style="display: flex; gap: 8px; margin-bottom: 8px;">
                    <select id="add-student-grade" style="flex: 1; padding: 10px; border: 1px solid #DFE6E9; border-radius: 10px;">
                    ...
                    <!-- 하루 학습 단어 수 선택 드롭다운 -->
                    <select id="add-student-word-count" style="flex: 1; padding: 10px; border: 1px solid #DFE6E9; border-radius: 10px; font-weight: bold; background: #FFF9E6;">
                        <option value="10">하루 10개 단어</option>
                        <option value="20" selected>하루 20개 단어</option>
                        <option value="30">하루 30개 단어</option>
                        <option value="50">하루 50개 단어</option>
                        <option value="100">하루 100개 단어</option>
                    </select>
                </div>
            ...
            <div id="admin-student-list" style="margin-bottom: 20px;">
                ${studentList.map((s) => `
                    <div style="background: #FFF; border: 1px solid #E9ECEF; padding: 12px 14px; border-radius: 12px; margin-bottom: 8px;">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <div>
                                <strong style="font-size: 16px;">${s.name}</strong> (${s.grade})
                        ...
                        <div style="margin-top: 8px; display: flex; align-items: center; gap: 8px;">
                            <span style="font-size: 12px; color: #7F8C8D; font-weight: bold;">하루 학습 단어 수 변경:</span>
                            <select class="change-word-count-select" data-id="${s.id}" style="padding: 4px 8px; border-radius: 8px; border: 1px solid #CBD5E0; font-size: 12px; font-weight: bold; background: #FFF9E6;">
                                <option value="10" ${s.dailyWordCount == 10 ? 'selected' : ''}>10개</option>
                                <option value="20" ${(!s.dailyWordCount || s.dailyWordCount == 20) ? 'selected' : ''}>20개</option>
                                <option value="30" ${s.dailyWordCount == 30 ? 'selected' : ''}>30개</option>
                                <option value="50" ${s.dailyWordCount == 50 ? 'selected' : ''}>50개</option>
                                <option value="100" ${s.dailyWordCount == 100 ? 'selected' : ''}>100개</option>
                            </select>
                        </div>
                    </div>
                `).join('')}
    ...
    document.getElementById('btn-add-student-submit').addEventListener('click', () => {
        ...
        const dailyWordCount = parseInt(document.getElementById('add-student-word-count').value);
        ...
        studentList.push({ id: newId, name, grade, pin, dailyWordCount });
        saveStudentsToStorage();
        alert(`✅ '${name}' 학생 (하루 ${dailyWordCount}개 단어 목표)이 추가되었습니다!`);
        renderAdminUI(modal);
    });

    // 기존 학생 하루 단어 수 즉시 변경 이벤트
    const changeCountSelects = modal.querySelectorAll('.change-word-count-select');
    changeCountSelects.forEach(select => {
        select.addEventListener('change', (e) => {
            const studentId = e.target.getAttribute('data-id');
            const newCount = parseInt(e.target.value);
            const targetStudent = studentList.find(s => s.id === studentId);
            if (targetStudent) {
                targetStudent.dailyWordCount = newCount;
                saveStudentsToStorage();
                
                // 만약 현재 로그인된 학생이라면 현재 세션도 업데이트
                if (currentUser && currentUser.id === studentId) {
                    currentUser.dailyWordCount = newCount;
                    saveCurrentUserToStorage(currentUser);
                }
                alert(`✅ ${targetStudent.name} 학생의 하루 학습 목표가 ${newCount}개 단어로 변경되었습니다!`);
            }
        });
    });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (currentUser) {
        const exists = studentList.some(s => s.id === currentUser.id);
        if (!exists) {
            saveCurrentUserToStorage(null);
        }
    }
